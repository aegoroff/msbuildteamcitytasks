...
namespace MSBuild.TeamCity.Tasks
{
    ...
    ///  />
    ///  ]]></code>
    ///     Runs tests and imports test results into TC using verbose output into build log and user defined action if no data
    ///     present
    ///     <code><![CDATA[
    ///  <Exec
    ///      Command="TestExecutable.exe --gtest_output=xml:"
    ...
    public class ImportGoogleTests : TeamCityTask
    {
        ...
        {
            var importer = new GoogleTestsPlainImporter(this.Logger, this.ContinueOnFailures, this.TestResultsPath)
            {
                Verbose = this.Verbose,
                WhenNoDataPublished = this.WhenNoDataPublished,
                ParseOutOfDate = this.ParseOutOfDate
            };
            this.status = importer.Import();
