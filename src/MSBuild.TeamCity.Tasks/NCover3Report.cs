...
namespace MSBuild.TeamCity.Tasks
{
    ...
    ///  />
    ///  ]]></code>
    ///     Configures .NET coverage processing using NCover3 tool, verbose output to log and user defined action on no data
    ///     present
    ///     <code><![CDATA[
    ///  <NCover3Report
    ///     ToolPath="C:\Program Files\NCover3"
    ...
    public class NCover3Report : TeamCityTask
    {
        ...
        {
            yield return new DotNetCoverMessage(DotNetCoverMessage.NCover3HomeKey, this.ToolPath);
            if (!string.IsNullOrEmpty(this.Arguments))
            {
                yield return new DotNetCoverMessage(DotNetCoverMessage.NCover3ReporterArgsKey, this.Arguments);
            }
